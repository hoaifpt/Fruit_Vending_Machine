...
2. **Compute the slug**:
   ```bash
   node .github/skills/fk/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
node .github/skills/fk/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node .github/skills/fk/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `fk` console messages, then stop the live server.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `.github/copilot-instructions.md` contains a `## Design Context` section from `fk init`, also generate 1-2 project-specific personas from the audience/brand info.
...
2. **Pass the structured metadata** through `FK_SKILLS_CRITIQUE_META` (JSON), then run the write command:
   ...
   FK_SKILLS_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .github/skills/fk/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .github/skills/fk/scripts/critique-storage.mjs trend <slug> 5
   ```
...
If `.github/copilot-instructions.md` contains a `## Design Context` section (generated by `fk init`), derive 1–2 additional personas from the audience and brand information:
